...
from core.utils import is_valid_date, parse_amount
...
class PageTypeSelection(QWizardPage):
    def __init__(self) -> None:
        ...
        self.combo_type.addItems(["Kiadás", "Bevétel"])
    ...
    def get_type(self) -> str:
        return "expense" if self.combo_type.currentText() == "Kiadás" else "income"
...
class PageAmount(QWizardPage):
    def __init__(self) -> None:
        super().__init__()
        self.setTitle("4/5. Összeg Rögzítése")
    ...
    def validatePage(self) -> bool:
        """Ellenőrzi, hogy az összeg valid szám és pozitív."""
    ...
    def get_amount(self) -> float:
        """Az összeget tiszta lebegőpontos számként adja vissza (pozitív)."""
        amount_str = self.input_amount.text().strip()
    ...
    def initializePage(self) -> None:
        ...
        # 1 tételes út: csak "Rögzítés" legyen, ne legyen "Tovább"
        wiz.button(QWizard.WizardButton.NextButton).setVisible(False)
        wiz.button(QWizard.WizardButton.FinishButton).setVisible(True)
        wiz.setButtonText(QWizard.WizardButton.FinishButton, "Rögzítés")

    def nextId(self) -> int:
        # Innen ne lehessen továbbmenni (csak Finish)
        return -1
...
class TransactionWizard(QWizard):
    ...
    def accept(self) -> None:
        """Mentés a 'Befejezés' gomb megnyomásakor."""
        # 0) Wizard mezők
        t_type = self.page(0).get_type()  # 'income' / 'expense'
        ...
        # 2) B-modell: amount mindig pozitív
        amount = abs(amount)

        # 3) Főablak elérése (kör-import elkerülése)
        parent = self.parent()
        if (
            parent is None
            or not hasattr(parent, "db")
            or not hasattr(parent, "set_page")
        ):
            QMessageBox.critical(self, "Hiba", "Nem található a főablak (MainWindow).")
            return
        ...
        if has_details:
            ...
            # ha nincs külön has_details fielded, ez legyen a has_details definíciója:
            # has_details = bool(details_text)
            ...
            for line in details_text.splitlines():
                ...
                try:
                    ...
                    # rossz sor: kihagyjuk (PageDetails.validatePage már jelzi)
                    continue

                self.db.add_transaction_item(
                    tx_id,
                    item_date=date,  # a fő tranzakció dátuma
                    item_name=item_name,
                    category_name=None,  # vagy a fő kategória neve, ha akarod
                    unit_price=float(unit_price),
                    quantity=float(quantity),
                    amount=float(item_amount),
                )

            # opcionális: ha add_transaction_item már sync-el, ez nem kell
            # parent.db.finalize_transaction_from_items(tx_id)
