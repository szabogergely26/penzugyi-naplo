...
from ui.bills.bill_card import BillCard
from ui.bills.bill_models import BillCardModel, MonthlyAmount, PeriodicAmount
...
class BillsPage(QWidget):
    ...
    def _load_demo_data_for_year(self, year: int) -> list[BillCardModel]:
        # Itt csak azt demonstráljuk, hogy év szerint változhatnak az összegek.
        # DB-nél: bills + bill_entries WHERE year = ?
        ...
        vidanet = BillCardModel(
            id=2,
            name="Vidanet / KalászNet",
            kind="monthly",
            monthly=[
        ...
        mvm = BillCardModel(
            ...
            periodic=[
                ...
                PeriodicAmount(f"{year}-02-01", f"{year}-03-01", 0),  # üres/0 eset
                PeriodicAmount(f"{year}-03-01", f"{year}-04-01", 19880),
            ],
        )

        return [telekom, vidanet, mvm]
